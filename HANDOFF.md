...
- Android v1 will ship the non-consumable `support_pack` through normal Google Play Billing, with no external billing. It unlocks only the Music Room and all 21 in-game BGM tracks. On 2026-08-09 the existing `standard` Buy purchase option was confirmed active at JPY 500 in Japan with prices and availability in all 173 Google Play countries/regions; the five localized names/descriptions were saved. Google Play Games Level Up may be revisited after release for lower service fees, but does not block v1.
...
- Android ranking is configured: `GodotPlayGamesServices` is registered as a Godot Android plugin singleton, uses `play-services-games-v2:21.0.0`, and supports sign-in, score submit, and leaderboard UI. The bundled Play Games project ID and seven leaderboard IDs are the current configured values; do not replace them without explicit evidence. See `RANKING_SETUP_GUIDE.md`.
...
- Validate `support_pack` purchase/cancel/pending/restore/reinstall/refund flows from a signed Internal testing install. The product and `standard` Buy purchase option are active at JPY 500 in Japan with all 173 Google Play countries/regions available and five localized descriptions saved; a sideloaded debug package is not valid billing evidence.
...
- If iOS also adopts the support-pack IAP for v1, have the individual Account Holder activate the Paid Apps Agreement and finish banking/tax information before sandbox validation. Add the first `support_pack` IAP to the same v1 app-version submission and do not release until it is approved and publishable. Verify that EU member-state storefronts are not excluded from v1 on either store, and separately obtain acceptance of the already configured Google public developer phone and public Payments/D-U-N-S address, or replace the phone with an explicitly approved durable business number.
