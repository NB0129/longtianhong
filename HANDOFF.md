...
The Apple organization-enrollment attempt failed, and the founder decided on 2026-07-22 to release v1 through the existing individual Apple Developer Program membership instead of blocking on organization conversion. Apple states that an individual member's personal legal name is shown as the App Store seller and developer name. The developer name is fixed when that account creates its first app record; if the membership already has an app record, capture the existing fixed value instead of assuming it can be changed for this app. The founder explicitly accepts public display of the legal name and has a virtual-office address plus a company-domain email prepared as public-contact candidates. The address is not approved evidence until it passes the applicable Apple document check or matches the Google Payments/D-U-N-S profile. Record the exact public preview before submission. To avoid the Apple EU DSA trader phone display, the founder decided to exclude all EU member-state storefronts from v1; the selected non-EU storefronts and Support URL still require final public-preview and local-requirement checks. EU distribution is a later, separately approved scope. Organization conversion may be requested later, but it is not a v1 dependency. The Google Play account is already an organization account; the founder has confirmed production access with another app on that same account, so the 12-testers/14-days requirement for newly created personal accounts is not a mandatory gate for this app. The founder also excluded the EU from the Google v1 business scope, but that does **not** remove Google's account-wide public-contact requirement: an organization developer account must show a verified developer phone number and email on Google Play independent of EU availability, and its public legal organization name/address must match the linked Payments/D-U-N-S data. A durable public business phone number and the actual public address preview are therefore unresolved Google release gates; do not substitute the founder's private mobile number without explicit approval. Normal internal/device QA remains required. Independently of the first-release date, complete Google developer identity verification and register `com.nb0129.machiate` by 2026-09-30; if releasing after that deadline, both are pre-submission gates.
...
- If the support-pack IAP ships in v1, have the individual Account Holder activate the Paid Apps Agreement and finish banking/tax information before sandbox validation. Add the first `support_pack` IAP to the same v1 app-version submission and do not release until it is approved and publishable. Verify that every EU member-state storefront remains excluded from v1 on both stores, and separately resolve and verify the public business phone and public Payments/D-U-N-S address required by the Google organization developer profile.
